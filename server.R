  ...
  div(class = "genderage",
      ...
      div(
        div("Median age",
            ...
            font-weight: bold; padding: 7px 10px; text-align: center;"),
          div(class = "age",
          tags$img(src = "img/Birthday_cake.png",
      ...
      div(
        div("Satisfaction rate",
            ...
            font-weight: bold;padding: 7px 10px; text-align: center"),
      div(class = "age",
          tags$img(src = "img/satisfaction.png",
                   width = "auto", height = "60%", style = "float:left;"),
...
customImageValueBox <- function(value, subtitle, img, imageHeight = 4, color = "aqua", width = NULL, href = NULL) {
  shinydashboard::valueBox(div(value, style = "margin:10px;
                           ...
                           icon = tags$i(style = "position: relative; top: 0.03vw; bottom: 0.03vw;",
                                         tags$img(src = img, style = paste0("height:",
...
#### Side bar------  
  ...
  output$response_rate <- renderUI({
    div(paste0(round2(filtered_data()$RESPONSE_RATE*100, 0), '%'), style = "text-align: left; vertical-align:middle; font-size:1.9vw; font-weight: bold; padding:10px;", br(),
      ...
      div(class = "genderage",
          div(
            div("Gender", style = "color:#2A64AB;font-size: calc(9px + 1vw);
                font-weight: bold;padding: 7px 10px; text-align: center"),
          div(class = "imgtext",
              tags$img(src = "img/People.png", width = "auto", height = "60%",
          ...
          div(
            div("Median age", style = "color:#2A64AB;font-size: calc(9px + 1vw);
                font-weight: bold;padding: 7px 10px; text-align: center"),
            div(class = "age",
                tags$img(src = "img/Birthday_cake.png", width = "auto", height = "60%",
                         style="float:left;"),
  ...
  output$quality_education <- renderUI ({
    ...
    if (input$student_group == "Bachelor's Degree") {
      box_wrapper(paste0(round2(filtered_data()$instruction * 100, 0),"%"),
  ...
  output$employed_full_time <- renderUI ({
    ...
    if (nchar(employed_full_time)> 4) {
      box_wrapper_blue_text(employed_full_time,
                            HTML("of employed respondents","<br>","were", "<b>", "working full-time", "<b/>"))
    } else {
      box_wrapper_blue(employed_full_time, 
                       HTML("of employed respondents","<br>", "were", "<b>", "working full-time", "<b/>"))
    }
  })
  # further studies
  ...
  output$related_program <- renderUI ({
    ...
    box_wrapper(paste0(round2(filtered_data()$JOB_PROGRAM_RELATED*100,0),"%"),
  ...
  # reactive value - plot for different skills
    ...
    Skills_Development<- c("Read and comprehend material", "Write clearly and concisely",
    ...
    values <- c(filtered_data()$Read, filtered_data()$Write_Clearly, filtered_data()$Speak_Clearly,
  ...
  output$labour<-renderUI({
    ...
    imageWrapper2(paste(round2(filtered_data()$IN_LABR_MKT*100, 0), "%", sep=""),
                  "In labour force", img = "img/Gears.png")
  })
  ...
  output$employment_rate<-renderUI({
    ...
    if (nchar(employed) < 5) {
      div(style = "background: #E0EBF6; margin-bottom: 8px; display: flex;
          align-items:center; justify-content:center;
      ...
      div(style = "background: #E0EBF6; margin-bottom: 8px;
          display: flex; align-items:center; justify-content:center;
          border-radius: 5px; border: 2px solid #E0EBF6; height:9vh;",
  ...
  output$related_program_emp <- renderUI({
    req(filtered_data()$JOB_PROGRAM_RELATED)
    box_wrapper_blue(paste0(round2(filtered_data()$JOB_PROGRAM_RELATED*100, 0), "%"),
  ...
  output$knowledge_skill_useful <- renderUI({
    ...
    box_wrapper(paste0(round2(filtered_data()$USEFUL_PERFORM_JOB*100, 0), "%"),
  ...
  output$text3 <- renderUI({
    div(HTML(paste("<em>", "Former students were asked how helpful their program was in their development of a number of skills.",
                   "<br>",
  ...
  output$text_skill <- renderUI({
    div(HTML(paste("<em>", "Select a skill from the dropdown menu on the right to view results by program area.", "<br>",
                   "Percentages are based on respondents who said their program was very helpful or helpful in developing the skill.",
                   "</em>", "</p>")),
        style = "text-align: center;")
  })
  ...
  output$text_related_job <- renderUI({
    div(HTML(paste("<em>", "Percentages are based on employed respondents who said their main job was very related or somewhat related to their program.",
                   "</em>")),
        style = "text-align: center;")
  })
  # text for knowledge and skills
  output$text_knowledge_skill <- renderUI({
    div(HTML(paste("<em>", "Percentages are based on employed respondents who said the knowledge and skills they gained were very useful or somewhat useful in performing their job.",
                   "</em>")),
        style = "text-align: center;")
  })
  ...
  output$text1 <- renderUI({
    HTML("<em>", "Former post-secondary students were asked about their educational experiences, subsequent employment, and further studies. Here is a summary of the results:",
         "</em>")
  })
    ...
    plot_ly(x = data_plot_all_programs()$satisfaction_rate,
      ...
      layout(orientation = 'h',
             title = '', titlefont = list(
               family = "Myriad-Pro",
               size = 16),
             xaxis = list(title = "", tickformat = "%", zeroline = FALSE,
                          fixedrange = TRUE,  range = c(0, 1.1)),
             yaxis = list(title = "", fixedrange = TRUE),
             margin = list(l = 220, pad = 7)) %>% 
      add_annotations(xref = 'x', yref = 'y',
                      x = data_plot_all_programs()$satisfaction_rate, 
                      y = reorder(data_plot_all_programs()$PROGRAM_AREA_NAME2,
    ...
    req(data_plot_all_programs()$PROGRAM_AREA_NAME2)
      data_plot_all_programs() %>% 
        plot_ly(x = value_select_satisfaction(),
                 y = ~ reorder(data_plot_all_programs()$PROGRAM_AREA_NAME2,
                               value_select_satisfaction()), # or get(input$select)
                 type = 'bar',
                 color = ~I(data_plot_all_programs()$fillColor), hoverinfo = 'none') %>% 
        layout(title = '', titlefont = list(
          family = "Myriad-Pro",
  ...
  summary_results_program_name <- reactive({
    div(style = "padding: 5px; margin-left:10px;color: #2A64AB;",
        ...
        h5(HTML("<em>", paste0(input$student_group, ",", " ", input$institution, ",", " ",
                              input$program, ifelse(filtered_data()$PROGRAM_NAME == " ",
                                                    " ", ", Program Name:"), " ",
                              filtered_data()$PROGRAM_NAME), "</em>"))
    )
  })
  
  summary_results_program_area <- reactive({
    div(style = "padding: 5px; margin-left:10px;color: #2A64AB;",
        h5(HTML("<em>", "<strong>", "Results are based on:", "</strong>",
                "</em>")),
        h5(HTML("<em>", paste0(input$student_group, ",", " ",
                               input$institution, ",", " ",
